...
- (void)didLoadFromCCB {
    ...
    //add the grounds, clouds and bushes to their respective arrays
    _grounds = @[_ground1, _ground2];
    _clouds = @[_cloud1, _cloud2];
    _bushes = @[_bush1, _bush2];
    
    //initialize the parallaxBackground and add it to the parallaxContainer
    _parallaxBackground = [CCParallaxNode node];
    [_parallaxContainer addChild:_parallaxBackground];
    
    //The bush ratio is larger than the cloud ratio because of the perceived difference in distance that we are trying to accomplish
    
    _bushParallaxRatio = ccp(.9, 1);
    _cloudParallaxRatio = ccp(.5, 1);
